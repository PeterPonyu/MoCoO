#!/usr/bin/env python
"""MoCoO Supplementary Figure S2 — FM-enhanced per-dataset metric profiles.

Same layout as Figure 3 but includes all 12 configurations (6 base + 6 FM
variants).  Highlights the effect of Phase-2 Flow Matching refinement on
the proposed metric set across datasets.

Layout:  2 rows × 4 columns (proposed 8 metrics)
...
def build_figure(results_dir: Path, outdir: Path):
    ...
    # Legend on the right margin — 12 configs need compact layout
    ...
    outpath = outdir / "figS2_fm_metric_profiles.png"
    issues = save_figure(
        fig, outpath, vcd_label="figS2_fm_metric_profiles", vcd_verbose=True,
    )
