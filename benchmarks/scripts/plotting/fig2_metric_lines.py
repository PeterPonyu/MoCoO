#!/usr/bin/env python
"""MoCoO Figure 2 — Per-dataset metric profiles (whole split).

Focused line-plot figure showing key metrics across datasets (whole split
only).  Each row is one metric family (Clustering, LSE, DRE-UMAP,
DRE-tSNE) with representative summary metrics — far fewer subplots than
the full 84-panel grid, making cross-config comparisons readable.

Layout:  4 rows × 3 columns  (12 subplots total)
  Row 1: ARI, NMI, ASW                   (Clustering)
  Row 2: LSE Overall, LSE Core, LSE Dim  (Latent quality)
  Row 3: DRE UMAP, UMAP Qloc, UMAP dCor (UMAP embedding)
  Row 4: DRE tSNE, tSNE Qloc, tSNE dCor (tSNE embedding)
...
# 4 rows × 3 columns of key metrics
# fmt: off
_ROWS = [
    ("Clustering", [
        ("ARI", "ARI \u2191"),
    ...
    ("LSE", [
        ...
        ("LSE_core_quality",            "LSE Core \u2191"),
        ("LSE_manifold_dimensionality", "LSE Dim \u2191"),
    ]),
    ("DRE \u2014 UMAP", [
        ("DRE_umap_overall_quality",      "DRE UMAP \u2191"),
        ("DRE_umap_Q_local",              "UMAP $Q_l$ \u2191"),
        ("DRE_umap_distance_correlation", "UMAP dCor \u2191"),
    ]),
    ("DRE \u2014 tSNE", [
        ("DRE_tsne_overall_quality",      "DRE tSNE \u2191"),
        ("DRE_tsne_Q_local",              "tSNE $Q_l$ \u2191"),
...
def build_figure(results_dir: Path, outdir: Path):
    ...
    fig, axes = plt.subplots(
        nrows, ncols, figsize=_FIGURE_SIZE,
        gridspec_kw={"hspace": 0.42, "wspace": 0.22,
                     "left": 0.065, "right": 0.82,
                     "top": 0.93, "bottom": 0.10},
    )

    for ri, (row_label, metrics) in enumerate(_ROWS):
        for ci, (mk, title) in enumerate(metrics):
