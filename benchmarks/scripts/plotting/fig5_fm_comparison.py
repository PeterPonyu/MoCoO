...
Panels:
  (a) Clustering:  NMI, ARI, ASW, DAV   (× 4 splits)
  (b) Quality:     DRE, LSE, DREX, LSEX (× 4 splits)
"""
...
_DATASET_ORDER = [
    "endo", "setty",
    "paul", "IRALL", "hemato",
    "dentate", "spinoids", "astrocyte",
    "lung", "retina", "teeth", "spine",
    "hepatoblastoma", "brainmet",
    "breast", "gastric", "livercancer",
    "melanoma", "pituitary", "hESCtime",
]
...
def _build_composed_axes(fig):
    ...
    return [
        (_PANEL_A, _make_block_axes(a_left, top_edge, a_cols)),
        (_PANEL_B, _make_block_axes(b_left, top_edge, b_cols)),
    ]
...
def build_figure(results_dir: Path, outdir: Path):
    ...
    for idx, ((_, _), axes) in enumerate(block_list):
        ...
        fig.text(
            pos.x0, pos.y1 + 0.040, panel_labels[idx],
    ...
    # Place legend at top center, wrapping to 2 rows if many configs
    ncol = min(6, len(active_configs))
    fig.legend(
    ...
    outpath = outdir / "fig5_fm_comparison.png"
    issues = save_figure(
        fig, outpath, vcd_label="fig5_fm_comparison", vcd_verbose=True,
    )
