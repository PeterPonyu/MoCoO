...
At 3{,}000 cells and 200 epochs, VAE+ODE achieves the most metric wins overall (13/27 metric--$\beta$ combinations in Table~\ref{tab:win_matrix}), reflecting the strong regularising effect of Neural ODE on the latent space. The VAE alone wins only 1/27 at full training scale, indicating that additional components are necessary.
...
\textbf{1. ODE is the dominant component.} Neural ODE consistently reduces DB by $0.28$--$0.62$ across all $\beta$ settings and wins 13/27 metric--$\beta$ combinations (Table~\ref{tab:win_matrix}). Its trajectory regularisation produces the most reliably improved latent spaces.
