...
\begin{figure*}[!t]
    ...
    \caption{Quantitative latent-space comparison across six configurations (IRALL, 3{,}000 cells, 200 epochs, $\beta{=}0.1$). \textbf{(A)}~UMAP projections coloured by cell type show progressive geometric refinement from VAE to Full. \textbf{(B)}~Clustering metrics (ARI, NMI, ASW) on validation and held-out test splits; solid and hatched bars respectively. \textbf{(C)}~Neighbourhood-preservation metrics (trustworthiness, neighbour symmetry, distance Spearman~$\rho$, DRE). \textbf{(D)}~Intrinsic latent-space quality (participation ratio, manifold dimensionality, anisotropy, composite LSE). Bold outlines mark the best configuration per metric.}
...
\begin{figure*}[!t]
    ...
    \caption{Ablation study and component contribution analysis (IRALL, $\beta{=}0.1$). \textbf{(A)}~ODE$\times$MoCo interaction heatmap across metrics and $\beta$ values; warm tones indicate super-additive effects. An adjacent summary table reports key metrics per configuration. \textbf{(B)}~Incremental-gain waterfall for ARI, NMI, and ASW: each bar shows the marginal change ($\Delta$) from the VAE baseline (green$=$positive, red$=$negative). \textbf{(C)}~Column-normalised performance heatmap across eight metrics---no single configuration dominates all axes. \textbf{(D)}~Permutation-based kNN accuracy drop per latent dimension ($d{=}32$), revealing how concentrated versus distributed each model's learned representations are.}
...
\begin{figure*}[!t]
    ...
    \caption{Training dynamics and computational cost (IRALL, 3{,}000 cells, $\beta{=}0.1$). \textbf{(A)}~Training (solid) and validation (dashed) ELBO loss curves; all configurations converge within 200 epochs (patience$=$40). ODE-containing models start from higher loss but reach comparable minima. \textbf{(B)}~Efficiency scatter: validation ARI versus wall-clock training time. Bubble area is proportional to peak GPU memory. The dashed Pareto frontier delineates the accuracy--cost trade-off: VAE achieves the best ARI at lowest cost, while ODE models require $\sim$10$\times$ longer for comparable clustering quality.}
