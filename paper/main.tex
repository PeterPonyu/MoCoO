...
We evaluate all six configurations on the IRALL hematopoiesis dataset under two experimental protocols. \textbf{Beta sweep} (Tables~\ref{tab:beta1}--\ref{tab:beta001}): 1{,}000 cells, 50 epochs, $\beta \in \{1.0, 0.1, 0.01\}$, targeting component interaction analysis. \textbf{Full-scale evaluation} (Figs.~\ref{fig:quant_comparison},~\ref{fig:training_dynamics},~\ref{fig:composed_benchmark}): 3{,}000 cells, 200 epochs, $\beta = 0.1$, providing higher-resolution latent spaces for UMAP visualisation, training dynamics, and comprehensive metric comparison. All genes are filtered to 2{,}000 HVGs. Metrics are computed on \textbf{all cells} via KMeans re-clustering of the learned latent space.

\subsection{Quantitative Comparison --- Beta Sweep}
\label{sec:results:quant}

Tables~\ref{tab:beta1}--\ref{tab:beta001} present the full quantitative comparison across all six configurations at each KL weight setting (1{,}000 cells). Fig.~\ref{fig:quant_comparison} provides a complementary visual comparison at the full-scale setting (3{,}000 cells, $\beta = 0.1$).
