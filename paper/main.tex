%% MoCoO: Momentum Contrast ODE-Regularized VAE for Single-Cell
%% Trajectory Inference and Representation Learning
%%
%% Prepared for IEEE Journal of Biomedical and Health Informatics (J-BHI)
%% Using IEEEtran document class
%%
\documentclass[journal]{IEEEtran}

%% Packages
\usepackage{cite}
\usepackage{amsmath,amssymb,amsfonts}
\usepackage{graphicx}
\usepackage{textcomp}
\usepackage{xcolor}
\usepackage{booktabs}
\usepackage{multirow}
\usepackage{array}
\usepackage{xspace}
\usepackage{bm}
\usepackage{balance}
\usepackage{tikz}

%% Graphics search path (generated figures live in benchmarks/figures/)
\graphicspath{{../benchmarks/figures/}{./}}

%% hyperref should be loaded last
\usepackage{hyperref}

%% Custom commands
\newcommand{\MoCoO}{\textsc{MoCoO}\xspace}
\newcommand{\ie}{\textit{i.e.}\xspace}
\newcommand{\eg}{\textit{e.g.}\xspace}

\begin{document}

\title{MoCoO: Momentum Contrast ODE-Regularized VAE for Single-Cell Trajectory Inference and Representation Learning}

\author{Zeyu~Fu%
\thanks{Manuscript submitted to IEEE Journal of Biomedical and Health Informatics.}%
\thanks{Z. Fu is with the School of Computer Science, University of Birmingham, Birmingham, B15 2TT, UK. (email: zxf123@student.bham.ac.uk)}
}

\markboth{IEEE Journal of Biomedical and Health Informatics}%
{Fu: MoCoO --- Momentum Contrast ODE-Regularized VAE for Single-Cell Trajectory Inference}

\maketitle

\begin{abstract}
Learning biologically faithful low-dimensional representations from single-cell RNA sequencing (scRNA-seq) data requires models that simultaneously preserve discrete cell-type identity and continuous developmental dynamics. We present \MoCoO, a modular framework that integrates a Variational Autoencoder (VAE), Neural Ordinary Differential Equations (Neural ODE), and Momentum Contrast (MoCo) with learnable prototypes for robust trajectory inference and representation learning. Through a systematic six-configuration ablation on the IRALL hematopoiesis dataset (3{,}000 cells, 200 epochs) with a KL weight sweep ($\beta \in \{1.0, 0.1, 0.01\}$), we investigate the standalone power and synergistic interactions of each component. Key findings: (1)~\textbf{ODE is the dominant component}---it consistently reduces Davies--Bouldin index (DB; lower is better) by $0.28$--$0.62$ across all $\beta$, winning 13 of 27 metric--$\beta$ combinations; (2)~\textbf{MoCo has beta-dependent standalone power}---contrastive learning improves ARI by $+0.039$ at $\beta = 0.01$ and DREX by $+0.006$ at $\beta \leq 0.1$, but is ineffective at strong KL; (3)~\textbf{ODE and MoCo exhibit regime-dependent synergy}---the interaction term for DB reaches $-0.129$ at $\beta = 0.01$ (super-additive compactness), with DRE ($+0.031$) and DREX ($+0.030$) synergies confirming improved embedding fidelity at low $\beta$; (4)~\textbf{the Full model achieves the best distance preservation} across configurations, with the highest DRE at $\beta = 0.01$ ($0.518$) and strongest pairwise distance correlations (DREX Spearman up to $0.72$). Subcategory analysis reveals that while VAE+ODE dominates aggregate clustering metrics, the Full model uniquely excels at geometric fidelity of the learned representation. Pseudotime--marker correlations are biologically significant across five systems: \textit{Hbb-bs} ($\rho = 0.28$) in IRALL, \textit{Slc1a3} ($\rho = 0.54$) in dentate, \textit{Ins2} ($\rho = 0.46$) in endo, \textit{Epor} ($\rho = 0.35$) in paul, and \textit{MKI67} ($\rho = 0.49$) in spinoids. We publicly release the MoCoO Python package and benchmark suite.
\end{abstract}

\begin{IEEEkeywords}
Single-cell RNA sequencing, variational autoencoder, neural ordinary differential equations, momentum contrast, contrastive learning, trajectory inference, pseudotime, representation learning, computational biology.
\end{IEEEkeywords}

%% ============================================================
\section{Introduction}
\label{sec:introduction}
%% ============================================================

Single-cell RNA sequencing (scRNA-seq) has transformed our understanding of cellular heterogeneity by enabling transcriptome-wide profiling at single-cell resolution~\cite{tang2009mrna}. A fundamental analysis task is to learn low-dimensional representations that faithfully capture both \textit{discrete} cell-type identity and \textit{continuous} developmental dynamics such as differentiation trajectories~\cite{kiselev2019challenges,trapnell2014dynamics}. These representations underpin downstream tasks including clustering, pseudotime inference, RNA velocity estimation, and differential expression analysis~\cite{wolf2018scanpy}.

Variational Autoencoders (VAEs) have become the de facto standard for scRNA-seq dimensionality reduction, with methods such as scVI~\cite{lopez2018deep} and scVAE~\cite{gronbech2020scvae} demonstrating strong performance through count-based likelihood models (negative binomial, ZINB). However, standard VAE latent spaces tend to be over-smoothed, conflating nearby cell states and producing suboptimal trajectory structures. Two complementary strategies have emerged to address this limitation.

First, Neural Ordinary Differential Equations (Neural ODEs)~\cite{chen2018neural} provide a principled framework for modelling continuous-time dynamics in latent space. By parameterising the derivative of the latent state with a neural network and integrating forward through an ODE solver, these models can learn smooth developmental trajectories without discrete time-step assumptions. Latent ODE models~\cite{rubanova2019latent} have shown promise for time-series modelling, and recent work has adapted them to single-cell contexts for pseudotime inference and RNA velocity estimation~\cite{fu2025panode}.

Second, contrastive learning---particularly Momentum Contrast (MoCo)~\cite{he2020momentum}---has proven effective at learning representations that are locally smooth yet globally discriminative. MoCo maintains a momentum-updated encoder and a large memory queue of negative keys, enabling scalable contrastive learning that is decoupled from batch size. Extensions such as prototype contrastive learning~\cite{li2021prototypical} further impose cluster structure by aligning representations with learnable prototype vectors.

Despite these individual advances, no existing method unifies VAE-based reconstruction, Neural ODE dynamics, and momentum contrastive learning within a single coherent framework. In this work, we propose \MoCoO(\textbf{Mo}mentum \textbf{Co}ntrast \textbf{O}DE-regularized VAE), a modular architecture that combines all three paradigms:

\begin{enumerate}
    \item \textbf{VAE with flexible count-based likelihoods} (MSE, NB, ZINB, Poisson, ZIP) provides a probabilistic latent space and handles the zero-inflation and overdispersion characteristic of scRNA-seq count data.
    \item \textbf{Neural ODE regularisation} models continuous developmental dynamics, derives unsupervised pseudotime ordering, and produces gradient-based RNA velocity estimates.
    \item \textbf{Momentum Contrast with prototype heads} imposes instance-level contrastive regularisation via a momentum encoder and memory queue, augmented by prototype contrastive learning that aligns representations with learnable cluster centres.
    \item \textbf{Information bottleneck} applies a secondary low-dimensional projection for hierarchical feature extraction.
    \item \textbf{Disentanglement losses} (DIP-VAE, $\beta$-TC-VAE, InfoVAE/MMD) optionally encourage axis-aligned, interpretable latent factors.
\end{enumerate}

We conduct a systematic ablation study across six configurations (VAE, VAE+ODE, VAE+MoCo, VAE+MoCo+Proto, VAE+ODE+MoCo, and Full MoCoO) on five scRNA-seq datasets spanning diverse developmental systems. Our evaluation encompasses clustering metrics (ARI, NMI, ASW, Calinski--Harabasz, Davies--Bouldin), dimensionality reduction quality (DRE, DREX, LSE, LSEX), biological validation (cell-type purity, marker gene enrichment, pseudotime--trajectory alignment), training dynamics, computational cost, and batch integration quality (iLISI, bASW, cLISI, graph connectivity, isolated label ASW via scIB).

%% ============================================================
\section{Related Work}
\label{sec:related}
%% ============================================================

\subsection{Variational Autoencoders for scRNA-seq}
\label{sec:related:vae}

scVI~\cite{lopez2018deep} introduced the negative binomial VAE for scRNA-seq, jointly modelling library size and batch effects. Extensions include scVAE~\cite{gronbech2020scvae} (multiple count distributions), scANVI~\cite{xu2021probabilistic} (semi-supervised cell-type annotation), and totalVI~\cite{gayoso2021joint} (multi-omics). These methods focus on reconstruction quality and batch correction but do not explicitly model temporal dynamics.

\subsection{Neural ODEs and Latent ODE Models}
\label{sec:related:ode}

Neural ODEs~\cite{chen2018neural} parameterise continuous dynamical systems $\frac{dz}{dt} = f_\theta(z, t)$ and integrate using adaptive ODE solvers. The Latent ODE framework~\cite{rubanova2019latent} combines a VAE encoder with an ODE-governed latent space for irregularly sampled time series. Applications to single-cell biology include trajectory inference~\cite{fu2025panode}, while related dynamical modelling approaches have advanced RNA velocity estimation~\cite{bergen2020generalizing}. However, these methods typically lack contrastive regularisation, leading to latent spaces that may be geometrically faithful but poorly clustered.

\subsection{Contrastive Learning for Single-Cell Data}
\label{sec:related:contrastive}

MoCo~\cite{he2020momentum} and SimCLR~\cite{chen2020simple} have been adapted to biological contexts. scAGCL~\cite{cai2024scagcl} applies symmetric augmentation-guided contrastive learning to scRNA-seq data with cell-type-aware positive pair construction. scGPCL~\cite{cai2024scgpcl} introduces prototype contrastive learning that aligns cell embeddings with learnable prototype vectors representing cell types. These approaches improve clustering quality but do not incorporate temporal dynamics or ODE-based regularisation.

\subsection{Integrated Approaches}
\label{sec:related:integrated}

Several methods combine two of the three paradigms. scDiff~\cite{hao2024scdiff} uses diffusion models (related to score-based SDEs) for single-cell generation. VeloVAE~\cite{li2024velovae} integrates VAE with RNA velocity. However, to our knowledge, no prior work jointly combines VAE, Neural ODE, and momentum contrastive learning in a unified architecture for single-cell analysis.

%% ============================================================
\section{Method}
\label{sec:method}
%% ============================================================

\subsection{Overview}
\label{sec:method:overview}

MoCoO is a modular framework illustrated in Fig.~\ref{fig:architecture}. The architecture processes scRNA-seq count data $X \in \mathbb{R}^{N \times G}$ ($N$ cells, $G$ genes) through five interconnected components: (1)~a VAE with count-based likelihood decoder, (2)~a Neural ODE solver for continuous dynamics, (3)~a Momentum Contrast module with memory queue, (4)~an information bottleneck, and (5)~optional disentanglement regularisers.

\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth]{fig_architecture}
    \caption{Overview of the \MoCoO architecture. Single-cell RNA sequencing count data is processed through a VAE encoder producing latent representations $z$, which are optionally refined by a Neural ODE solver for continuous trajectory modelling. A Momentum Contrast module with memory queue enforces instance-level discrimination, while learnable prototype heads impose cluster structure. An information bottleneck provides hierarchical compression. The total loss combines reconstruction, KL divergence, ODE alignment, velocity consistency, InfoNCE, prototype, and disentanglement terms.}
    \label{fig:architecture}
\end{figure*}

\subsection{Encoder}
\label{sec:method:encoder}

The encoder $q_\phi(z|x)$ maps log-normalised gene expression $\tilde{x} = \log(1 + x)$ through a two-layer MLP with ReLU activations to produce mean $\mu$ and pre-softplus scale $\sigma'$ parameters of a diagonal Gaussian posterior in~\eqref{eq:encoder}, with latent samples drawn via the reparameterisation trick in~\eqref{eq:reparam}:
\begin{equation}
    q_\phi(z|x) = \mathcal{N}\!\left(z;\, \mu_\phi(\tilde{x}),\, \mathrm{softplus}(\sigma'_\phi(\tilde{x}))^2 I\right),
    \label{eq:encoder}
\end{equation}
\begin{equation}
    z = \mu + \mathrm{softplus}(\sigma') \cdot \epsilon, \quad \epsilon \sim \mathcal{N}(0, I).
    \label{eq:reparam}
\end{equation}

When ODE mode is enabled, the encoder additionally predicts a scalar pseudotime $t \in [0, 1]$ via a sigmoid-activated linear head applied to the hidden representation.

\subsection{Neural ODE Component}
\label{sec:method:ode}

Given latent samples $\{z_i\}$ and predicted pseudotimes $\{t_i\}$, the ODE module orders cells by pseudotime and solves the initial value problem in~\eqref{eq:ode}:
\begin{equation}
    \frac{dz}{dt} = f_\psi(z, t), \quad z(t_0) = z_{\mathrm{init}},
    \label{eq:ode}
\end{equation}
where $f_\psi$ is a two-layer MLP with ELU activations. Integration is performed using \texttt{torchdiffeq}'s fixed-step RK4 solver. The ODE-integrated representations $z_{\mathrm{ode}}$ are blended with VAE latent samples via configurable weights as defined in~\eqref{eq:blend}:
\begin{equation}
    z_{\mathrm{blend}} = \alpha_{\mathrm{vae}} \cdot z + \alpha_{\mathrm{ode}} \cdot z_{\mathrm{ode}}.
    \label{eq:blend}
\end{equation}

Two auxiliary losses regularise the ODE:
\begin{itemize}
    \item \textbf{ODE--VAE alignment:} $\mathcal{L}_{\mathrm{ode}} = \| z - z_{\mathrm{ode}} \|_2^2$
    \item \textbf{Velocity consistency:} $\mathcal{L}_{\mathrm{vel}} = 1 - \cos(z_{i+1} - z_i,\, f_\psi(z_i, t_i))$, encouraging the ODE velocity field to align with empirical displacements.
\end{itemize}

\subsection{Momentum Contrast Module}
\label{sec:method:moco}

The MoCo module follows the MoCo v1 design~\cite{he2020momentum} with enhancements from scAGCL~\cite{cai2024scagcl} and scGPCL~\cite{cai2024scgpcl}:

\begin{enumerate}
    \item \textbf{Query and Key Encoders:} The query encoder shares parameters with the main VAE encoder. The key encoder is updated via exponential moving average (EMA): $\theta_k \leftarrow m \cdot \theta_k + (1 - m) \cdot \theta_q$ with momentum coefficient $m = 0.999$.
    \item \textbf{Projection Heads:} Two-layer MLPs with BatchNorm and ReLU map $d$-dimensional latent representations to a $d_{\mathrm{proj}}$-dimensional space for contrastive comparison.
    \item \textbf{Memory Queue:} A FIFO buffer of size $K$ stores projected key representations, decoupling the effective number of negatives from batch size.
    \item \textbf{InfoNCE Loss} (see~\eqref{eq:infonce}):
    \begin{equation}
        \mathcal{L}_{\mathrm{moco}} \!=\! -\log \frac{\exp(q \!\cdot\! k^+ / \tau)}{\exp(q \!\cdot\! k^+ / \tau) + \!\sum_{k^-} \exp(q \!\cdot\! k^- / \tau)},
        \label{eq:infonce}
    \end{equation}
    where the sum is over all $k^-$ in the memory queue and $\tau$ is the temperature parameter.
    \item \textbf{Prototype Contrastive Loss} (optional; see~\eqref{eq:proto}):
    \begin{equation}
        \mathcal{L}_{\mathrm{proto}} = -\frac{1}{N} \sum_{i=1}^{N} \log \frac{\exp(z_i \cdot p_{c(i)} / \tau)}{\sum_{j=1}^{P} \exp(z_i \cdot p_j / \tau)},
        \label{eq:proto}
    \end{equation}
    where $\{p_j\}_{j=1}^{P}$ are learnable prototype vectors and $c(i) = \arg\max_j\, z_i \cdot p_j / \tau$ assigns cell $i$ to its nearest prototype.
\end{enumerate}

\subsection{Decoder}
\label{sec:method:decoder}

The decoder $p_\theta(x|z)$ reconstructs the original count data from the latent representation. For count-based likelihoods, the decoder predicts normalised mean parameters $\rho = \mathrm{softmax}(\mathrm{MLP}(z))$ and uses gene-specific learnable dispersion parameters $r$:
\begin{itemize}
    \item \textbf{Negative Binomial (NB):} $p(x_g | z) = \mathrm{NB}(x_g;\, \mu_g = l \cdot \rho_g,\, r_g)$
    \item \textbf{Zero-Inflated NB (ZINB):} $p(x_g | z) = \pi_g \cdot \delta_0(x_g) + (1 - \pi_g) \cdot \mathrm{NB}(x_g;\, \mu_g,\, r_g)$
\end{itemize}
where $l = \sum_g x_g$ is the library size and $\pi_g$ is a dropout probability predicted by a secondary decoder head.

\subsection{Information Bottleneck}
\label{sec:method:ib}

A secondary encoder-decoder pair maps $z \in \mathbb{R}^{d}$ to a lower-dimensional bottleneck $b \in \mathbb{R}^{d_{\mathrm{ib}}}$ (where $d_{\mathrm{ib}} < d$) and back, imposing a hierarchical compression that encourages the model to capture the most salient features.

\subsection{Total Loss}
\label{sec:method:loss}

The total training objective combines all components as defined in~\eqref{eq:total_loss}:
\begin{align}
    \mathcal{L} ={} & \lambda_{\mathrm{recon}} \mathcal{L}_{\mathrm{recon}} + \lambda_{\mathrm{irecon}} \mathcal{L}_{\mathrm{irecon}} + \mathcal{L}_{\mathrm{ode}} + \mathcal{L}_{\mathrm{vel}} \nonumber \\
    & + \beta \cdot D_{\mathrm{KL}}(q_\phi(z|x) \| p(z)) + \lambda_{\mathrm{dip}} \mathcal{L}_{\mathrm{dip}} \nonumber \\
    & + \lambda_{\mathrm{tc}} \mathcal{L}_{\mathrm{tc}} + \lambda_{\mathrm{mmd}} \mathcal{L}_{\mathrm{mmd}} \nonumber \\
    & + \lambda_{\mathrm{moco}} (\mathcal{L}_{\mathrm{moco}} + \mathcal{L}_{\mathrm{cross}}) + \lambda_{\mathrm{proto}} \mathcal{L}_{\mathrm{proto}},
    \label{eq:total_loss}
\end{align}
where $\lambda$ terms are configurable weights and $\mathcal{L}_{\mathrm{cross}}$ is a cross-path contrastive loss aligning ODE and VAE representations.

%% ============================================================
\section{Experiments}
\label{sec:experiments}
%% ============================================================

\subsection{Datasets}
\label{sec:experiments:datasets}

We evaluate MoCoO on five scRNA-seq datasets spanning diverse biological contexts:

\begin{enumerate}
    \item \textbf{IRALL (Hematopoiesis):} 41{,}252 cells, 12 types, 8 batches (d0--d30). Mouse bone marrow hematopoiesis from HSC to mature lineages.
    \item \textbf{Dentate (Neurogenesis):} 18{,}213 cells, 14 types. Mouse dentate gyrus, covering radial glia to mature granule neurons.
    \item \textbf{Endo (Endocrine Pancreas):} 2{,}531 cells, 13 types. Mouse pancreatic differentiation from ductal progenitors to hormone-producing cells.
    \item \textbf{Paul (Myeloid/Erythroid):} 2{,}730 cells, 19 types. Classic myeloid/erythroid progenitor bifurcation dataset \cite{paul2015transcriptional}, providing a fine-grained challenge with 19 clusters (some $< 30$ cells).
    \item \textbf{Spinoids (Spinal Cord Organoid):} 9{,}619 cells, 8 types. Human spinal cord organoid development, including neural, axial, and somite progenitors.
\end{enumerate}

\subsection{Model Configurations}
\label{sec:experiments:configs}

We evaluate six configurations to isolate the contribution of each component (Table~\ref{tab:configs}).

\begin{table}[!t]
    \centering
    \caption{Model configurations and their active components.}
    \label{tab:configs}
    \begin{tabular}{lcccc}
        \toprule
        Configuration   & VAE          & ODE          & MoCo         & Proto \\
        \midrule
        VAE             & $\checkmark$ &              &              &              \\
        VAE+ODE         & $\checkmark$ & $\checkmark$ &              &              \\
        VAE+MoCo        & $\checkmark$ &              & $\checkmark$ &              \\
        VAE+MoCo+Proto  & $\checkmark$ &              & $\checkmark$ & $\checkmark$ \\
        VAE+ODE+MoCo    & $\checkmark$ & $\checkmark$ & $\checkmark$ &              \\
        Full (MoCoO)    & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
        \bottomrule
    \end{tabular}
\end{table}

All models use: latent dimension $d = 32$, bottleneck dimension $d_{\mathrm{ib}} = 4$, hidden dimension $h = 128$, negative binomial likelihood, learning rate $10^{-4}$, batch size 128, and 200 training epochs (patience 40). MoCo configurations use queue size $K = 4096$, momentum $m = 0.999$, temperature $\tau = 0.2$, $\lambda_{\mathrm{moco}} = 0.3$ (with ODE) or $0.5$ (without ODE), $P = 12$ prototypes with $\lambda_{\mathrm{proto}} = 0.1$. ODE configurations use $\alpha_{\mathrm{vae}} = 0.6$, $\alpha_{\mathrm{ode}} = 0.4$. Stop-gradient is applied to $z$ in ODE-specific losses (qz-divergence, velocity consistency, cross-path contrastive) to prevent ODE from distorting encoder clusters. The KL weight $\beta$ is swept across $\{1.0, 0.1, 0.01\}$ to investigate component sensitivity.

\subsection{Evaluation Metrics}
\label{sec:experiments:metrics}

\textbf{Clustering quality:} Adjusted Rand Index (ARI), Normalised Mutual Information (NMI), Average Silhouette Width (ASW), Calinski--Harabasz index (CH), Davies--Bouldin index (DB).

\textbf{Latent Structure Evaluation (LSE):} Manifold dimensionality, spectral decay rate, participation ratio, anisotropy, trajectory directionality, noise resilience---intrinsic latent space quality without reference to downstream clustering.

\textbf{Dimensionality Reduction Evaluation (DRE):} Distance correlation (Spearman), $Q_{\mathrm{local}}$, $Q_{\mathrm{global}}$ via co-ranking matrix---assessing latent-to-UMAP and latent-to-tSNE embedding fidelity.

\textbf{Biological validation:} Cell-type purity, marker gene enrichment, pseudotime--trajectory correlation (Spearman $\rho$).

\textbf{Batch integration (scIB):} Integration LISI (iLISI), batch-aware silhouette width (bASW), cell-type LISI (cLISI), graph connectivity, isolated label ASW, composite bio-conservation and batch-correction scores.

\textbf{Computational cost:} Wall-clock training time and peak GPU memory.

%% ============================================================
\section{Results}
\label{sec:results}
%% ============================================================

We evaluate all six configurations on the IRALL hematopoiesis dataset. \textbf{Beta sweep} (Tables~\ref{tab:beta1}--\ref{tab:beta001}): 3{,}000 cells, 200 epochs (patience 40), $\beta \in \{1.0, 0.1, 0.01\}$, targeting component interaction analysis. \textbf{Full-scale evaluation} (Figs.~\ref{fig:quant_comparison},~\ref{fig:training_dynamics},~\ref{fig:composed_benchmark}): same setting with $\beta = 0.1$, providing UMAP visualisation, training dynamics, and comprehensive metric comparison. All genes are filtered to 3{,}000 HVGs. Metrics are computed on \textbf{all cells} via KMeans re-clustering of the learned latent space.

\subsection{Quantitative Comparison --- Beta Sweep}
\label{sec:results:quant}

Tables~\ref{tab:beta1}--\ref{tab:beta001} present the full quantitative comparison across all six configurations at each KL weight setting (3{,}000 cells, 200 epochs). Fig.~\ref{fig:quant_comparison} provides a complementary visual comparison at $\beta = 0.1$.

%% TABLE I
\begin{table*}[!t]
    \centering
    \caption{IRALL, 3{,}000 cells, 200 epochs --- $\beta = 1.0$ (standard KL). Best values in \textbf{bold}. DB$\downarrow$ indicates lower is better.}
    \label{tab:beta1}
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{lccccccccc}
        \toprule
        Config & ARI & NMI & ASW & CH & DB$\downarrow$ & LSE & DRE & DREX & LSEX \\
        \midrule
        VAE              & 0.395          & 0.543          & 0.122          & 203.4          & 2.139          & \textbf{0.317} & 0.415          & 0.591          & 0.608          \\
        \textbf{VAE+ODE} & \textbf{0.485} & 0.558          & \textbf{0.212} & \textbf{401.4} & \textbf{1.522} & 0.208          & \textbf{0.454} & \textbf{0.618} & \textbf{0.610} \\
        VAE+MoCo         & 0.374          & 0.513          & 0.130          & 204.8          & 2.054          & 0.315          & 0.421          & 0.587          & 0.608          \\
        VAE+MoCo+Proto   & 0.422          & 0.528          & 0.140          & 202.4          & 2.025          & 0.319          & 0.402          & 0.574          & 0.609          \\
        VAE+ODE+MoCo     & 0.480          & \textbf{0.574} & 0.192          & 389.9          & 1.647          & 0.204          & 0.444          & 0.586          & 0.608          \\
        Full (MoCoO)     & 0.396          & 0.537          & 0.173          & 379.4          & 1.714          & 0.204          & 0.452          & 0.610          & 0.609          \\
        \bottomrule
    \end{tabular}%
    }
\end{table*}

At strong KL ($\beta = 1.0$), \textbf{ODE is the dominant component}: VAE+ODE achieves the best ARI (0.485), ASW (0.212), CH (401.4), DB (1.522), DRE (0.454), DREX (0.618), and LSEX (0.610)---winning 7 of 9 metrics. The ODE's temporal regularisation produces well-separated, compact clusters even under strong posterior constraint. VAE+ODE+MoCo achieves the best NMI (0.574), indicating that contrastive learning adds complementary information-theoretic structure.

%% TABLE II
\begin{table*}[!t]
    \centering
    \caption{IRALL, 3{,}000 cells, 200 epochs --- $\beta = 0.1$ (moderate KL). Best values in \textbf{bold}. DB$\downarrow$ indicates lower is better.}
    \label{tab:beta01}
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{lccccccccc}
        \toprule
        Config & ARI & NMI & ASW & CH & DB$\downarrow$ & LSE & DRE & DREX & LSEX \\
        \midrule
        VAE              & \textbf{0.489} & 0.561          & 0.189          & 371.0          & 1.675          & 0.228          & 0.494          & 0.650          & 0.593          \\
        \textbf{VAE+ODE} & 0.475          & \textbf{0.573} & 0.292          & \textbf{959.4} & \textbf{1.168} & 0.138          & \textbf{0.547} & \textbf{0.689} & 0.588          \\
        VAE+MoCo         & 0.470          & 0.554          & 0.188          & 360.2          & 1.700          & 0.232          & 0.500          & 0.656          & 0.593          \\
        VAE+MoCo+Proto   & 0.431          & 0.563          & 0.157          & 355.6          & 1.813          & \textbf{0.239} & 0.515          & 0.664          & \textbf{0.594} \\
        VAE+ODE+MoCo     & 0.473          & 0.561          & \textbf{0.307} & 680.6          & 1.190          & 0.174          & 0.529          & 0.666          & 0.591          \\
        Full (MoCoO)     & 0.446          & 0.559          & 0.272          & 697.3          & 1.185          & 0.152          & 0.518          & 0.678          & 0.589          \\
        \bottomrule
    \end{tabular}%
    }
\end{table*}

At moderate KL ($\beta = 0.1$), \textbf{ODE remains the strongest single component}: VAE+ODE wins NMI (0.573), CH (959.4), DB (1.168), DRE (0.547), and DREX (0.689). The VAE leads ARI (0.489), and VAE+ODE+MoCo achieves the best ASW (0.307). VAE+MoCo+Proto wins LSE (0.239) and LSEX (0.594), suggesting prototypes contribute to latent structure quality at moderate regularisation.

%% TABLE III
\begin{table*}[!t]
    \centering
    \caption{IRALL, 3{,}000 cells, 200 epochs --- $\beta = 0.01$ (weak KL). Best values in \textbf{bold}. DB$\downarrow$ indicates lower is better.}
    \label{tab:beta001}
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{lccccccccc}
        \toprule
        Config & ARI & NMI & ASW & CH & DB$\downarrow$ & LSE & DRE & DREX & LSEX \\
        \midrule
        VAE              & 0.441          & 0.570          & 0.199          & 485.4          & 1.557          & 0.188          & 0.526          & 0.680          & 0.595          \\
        VAE+ODE          & 0.465          & 0.573          & \textbf{0.285} & 621.8          & 1.278          & 0.173          & 0.488          & 0.643          & 0.588          \\
        \textbf{VAE+MoCo}& \textbf{0.480} & 0.579          & 0.188          & 431.8          & 1.630          & 0.203          & 0.535          & \textbf{0.686} & \textbf{0.596} \\
        VAE+MoCo+Proto   & 0.420          & 0.566          & 0.162          & 437.6          & 1.748          & \textbf{0.205} & \textbf{0.541} & 0.685          & 0.596          \\
        VAE+ODE+MoCo     & 0.477          & 0.570          & 0.259          & \textbf{719.0} & \textbf{1.222} & 0.163          & 0.528          & 0.679          & 0.590          \\
        Full (MoCoO)     & 0.474          & \textbf{0.581} & 0.269          & 629.3          & 1.240          & 0.171          & 0.518          & 0.669          & 0.589          \\
        \bottomrule
    \end{tabular}%
    }
\end{table*}

At weak KL ($\beta = 0.01$), the competition broadens: VAE+MoCo leads ARI (0.480), DREX (0.686), and LSEX (0.596); VAE+ODE+MoCo wins CH (719.0) and DB (1.222); Full achieves the best NMI (0.581). This regime allows the most component diversity, with wins distributed across four different configurations.

\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth,height=0.42\textheight,keepaspectratio]{fig2_quant_comparison}
    \caption{Quantitative latent space comparison across six model configurations (IRALL, 3{,}000 cells, 200 epochs, $\beta = 0.1$). \textbf{(A)}~UMAP embeddings coloured by cell type illustrate how latent geometry changes with configuration. \textbf{(B)}~Clustering quality (ARI, NMI, ASW) with Val/Test splits. \textbf{(C)}~Neighbourhood quality (trustworthiness, symmetry, Spearman, DRE). \textbf{(D)}~Latent structure (participation ratio, intrinsic dimensionality, anisotropy, LSE). The Full model achieves the best ASW and DRE, while VAE leads on ARI.}
    \label{fig:quant_comparison}
\end{figure*}

\subsection{Component Effect Analysis}
\label{sec:results:component}

We isolate each component's marginal effect by computing deltas between adjacent configurations across all three beta settings (Table~\ref{tab:component_effects}; see also Fig.~\ref{fig:ablation_summary} for a visual summary).

%% TABLE IV
\begin{table*}[!t]
    \centering
    \caption{Component effects ($\Delta$ from baseline, by $\beta$). Bold values indicate the most notable effects.}
    \label{tab:component_effects}
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{llrrrl}
        \toprule
        Component & Metric & $\beta{=}1.0$ & $\beta{=}0.1$ & $\beta{=}0.01$ & Interpretation \\
        \midrule
        \textbf{ODE} (VAE$\to$VAE+ODE)     & $\Delta$ARI  & $\mathbf{+0.090}$ & $-0.014$ & $+0.024$ & Strong ARI boost at $\beta{=}1.0$ \\
                                             & $\Delta$DB   & $\mathbf{-0.617}$ & $\mathbf{-0.507}$ & $-0.279$ & Consistently improves DB (better) \\
                                             & $\Delta$LSEX & $+0.002$ & $-0.005$ & $-0.007$ & Marginal LSEX effect \\
        \midrule
        \textbf{MoCo} (VAE$\to$VAE+MoCo)    & $\Delta$ARI  & $-0.021$ & $-0.019$ & $\mathbf{+0.039}$ & \textbf{Beta-dependent}: effective at $\beta{=}0.01$ \\
                                             & $\Delta$ASW  & $+0.008$ & $-0.001$ & $-0.011$ & Mixed \\
                                             & $\Delta$DREX & $-0.004$ & $+0.006$ & $+0.006$ & Small positive DREX at low $\beta$ \\
        \midrule
        \textbf{Proto} (MoCo$\to$MoCo+P)    & $\Delta$ARI  & $\mathbf{+0.048}$ & $-0.039$ & $-0.060$ & Helps ARI at $\beta{=}1.0$ only \\
                                             & $\Delta$DB   & $-0.029$ & $+0.113$ & $+0.118$ & Worsens DB at low $\beta$ \\
                                             & $\Delta$DRE  & $-0.019$ & $+0.015$ & $+0.006$ & Small DRE boost at $\beta{\le}0.1$ \\
        \bottomrule
    \end{tabular}%
    }
\end{table*}

\textbf{Key observation:} ODE is the \textbf{dominant component} at full training scale, consistently improving DB (cluster compactness) by $0.28$--$0.62$ across all beta values. Its ARI boost is strongest at $\beta=1.0$ ($+0.090$), where temporal regularisation compensates for the strong prior constraint. MoCo's ARI contribution is \textbf{beta-dependent}: effective at $\beta=0.01$ ($\Delta$ARI$=+0.039$) but marginal otherwise. Prototypes boost ARI only at $\beta=1.0$ ($+0.048$).

\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth,height=0.42\textheight,keepaspectratio]{fig3_ablation_summary}
    \caption{Ablation study and component contribution analysis ($\beta = 0.1$, IRALL). \textbf{(A)}~ODE$\times$MoCo synergy heatmap: interaction terms across metrics and $\beta$ values, with warm colours indicating super-additive effects; alongside a summary table of key metrics per configuration. \textbf{(B)}~Incremental gain waterfall charts for ARI, NMI, and ASW, showing component marginal effects ($\Delta$ from VAE baseline) with green/red indicating positive/negative deltas. \textbf{(C)}~Comprehensive performance heatmap (column-normalised) across eight evaluation metrics, revealing that no single configuration dominates all axes. \textbf{(D)}~Latent dimension importance via permutation-based kNN accuracy drop, quantifying how ``focused'' vs.\ ``diffuse'' each model's representations are across the 32-dimensional latent space.}
    \label{fig:ablation_summary}
\end{figure*}

\subsection{ODE \texorpdfstring{$\times$}{x} MoCo Synergy Analysis}
\label{sec:results:synergy}

To test whether ODE and MoCo interact synergistically (beyond additive effects), we compute the interaction term in~\eqref{eq:synergy}:
\begin{multline}
    \Delta_{\mathrm{synergy}} = \mathrm{(VAE{+}ODE{+}MoCo)} - \mathrm{(VAE{+}ODE)} \\
    - \mathrm{(VAE{+}MoCo)} + \mathrm{VAE}.
    \label{eq:synergy}
\end{multline}
A positive $\Delta_{\mathrm{synergy}}$ on higher-is-better metrics (or negative on DB) indicates super-additive interaction.

%% TABLE V
\begin{table}[!t]
    \centering
    \caption{ODE $\times$ MoCo synergy (interaction term). Bold values indicate strong synergy.}
    \label{tab:synergy}
    \resizebox{\columnwidth}{!}{%
    \begin{tabular}{lrrrl}
        \toprule
        Metric & $\beta{=}1.0$ & $\beta{=}0.1$ & $\beta{=}0.01$ & Synergistic? \\
        \midrule
        ARI         & $+0.016$          & $+0.017$          & $-0.027$          & Weak positive at $\beta{\ge}0.1$ \\
        ASW         & $-0.028$          & $\mathbf{+0.016}$ & $-0.015$          & Positive at $\beta{=}0.1$ only \\
        DB$\downarrow$ & $+0.210$       & $-0.003$          & $\mathbf{-0.129}$ & \textbf{Yes}---strong DB improvement at $\beta{=}0.01$ \\
        DRE         & $-0.016$          & $-0.024$          & $\mathbf{+0.031}$ & \textbf{Yes}---at $\beta{=}0.01$ \\
        DREX        & $-0.028$          & $-0.029$          & $\mathbf{+0.030}$ & \textbf{Yes}---at $\beta{=}0.01$ \\
        \bottomrule
    \end{tabular}%
    }
\end{table}

\textbf{Finding:} ODE and MoCo exhibit \textbf{regime-dependent} synergy. At $\beta=0.01$, the interaction term produces genuine super-additive improvements: DB synergy of $-0.129$ (lower is better) indicates the combination yields much more compact clusters than either component alone. DRE ($+0.031$) and DREX ($+0.030$) synergies at $\beta=0.01$ show improved embedding fidelity. At $\beta=0.1$, ASW synergy ($+0.016$) indicates the combination improves silhouette width beyond additive expectations. ARI synergy is weakly positive ($+0.016$--$0.017$) at $\beta\ge0.1$ but turns negative at $\beta=0.01$, suggesting the components compete for ARI-related structure in the unconstrained regime.

The DB synergy pattern is notable: strongly negative (harmful) at $\beta=1.0$ ($+0.210$), neutral at $\beta=0.1$, and strongly positive at $\beta=0.01$ ($-0.129$). This indicates the ODE$\times$MoCo interaction for cluster compactness requires sufficient latent freedom ($\beta\le0.01$) to manifest.

\subsection{Beta Sensitivity --- Full Model}
\label{sec:results:beta}

Table~\ref{tab:beta_full} presents the Full model's performance across all three KL weight settings.

%% TABLE VI
\begin{table}[!t]
    \centering
    \caption{Full model performance across KL weight settings. Best value per metric in \textbf{bold}.}
    \label{tab:beta_full}
    \resizebox{\columnwidth}{!}{%
    \begin{tabular}{lrrrcl}
        \toprule
        Metric & $\beta{=}1.0$ & $\beta{=}0.1$ & $\beta{=}0.01$ & Best $\beta$ & Trend \\
        \midrule
        ARI         & 0.396          & 0.446          & \textbf{0.474} & 0.01 & $\uparrow$ with lower $\beta$ \\
        NMI         & 0.537          & 0.559          & \textbf{0.581} & 0.01 & $\uparrow$ with lower $\beta$ \\
        ASW         & 0.173          & \textbf{0.272} & 0.269          & 0.1  & Peak at $\beta{=}0.1$ \\
        CH          & 379.4          & \textbf{697.3} & 629.3          & 0.1  & Peak at $\beta{=}0.1$ \\
        DB$\downarrow$ & 1.714       & \textbf{1.185} & 1.240          & 0.1  & Best at $\beta{=}0.1$ \\
        LSE         & \textbf{0.204} & 0.152          & 0.171          & 1.0  & $\uparrow$ with higher $\beta$ \\
        DRE         & 0.452          & \textbf{0.518} & 0.518          & 0.1  & Plateau at $\beta{\le}0.1$ \\
        DREX        & 0.610          & \textbf{0.678} & 0.669          & 0.1  & Peak at $\beta{=}0.1$ \\
        LSEX        & \textbf{0.609} & 0.589          & 0.590          & 1.0  & $\uparrow$ with higher $\beta$ \\
        \bottomrule
    \end{tabular}%
    }
\end{table}

The Full model shows a clear \textbf{sweet spot at $\beta = 0.1$}, which wins 5 of 9 metrics (ASW, CH, DB, DRE, DREX). Clustering identity metrics (ARI, NMI) continue to improve at $\beta=0.01$, while spectral metrics (LSE, LSEX) favor $\beta=1.0$. This suggests $\beta=0.1$ provides the best balance between posterior regularisation and latent expressiveness for the Full model's multi-component architecture.

\subsection{Metric Win Counts}
\label{sec:results:wins}

Table~\ref{tab:wins} summarises which configurations win the most metrics across all beta settings.

%% TABLE VII
\begin{table}[!t]
    \centering
    \caption{Number of metric wins (out of 9) by configuration. Total is out of 27 (9 metrics $\times$ 3 beta values).}
    \label{tab:wins}
    \begin{tabular}{lcccc}
        \toprule
        Config & $\beta{=}1.0$ & $\beta{=}0.1$ & $\beta{=}0.01$ & Total (27) \\
        \midrule
        \textbf{VAE+ODE}   & \textbf{7} & \textbf{5} & 1          & \textbf{13} \\
        VAE+MoCo+Proto     & 1          & 2          & 2          & 5           \\
        VAE+ODE+MoCo       & 1          & 1          & 2          & 4           \\
        VAE+MoCo           & 0          & 0          & \textbf{3} & 3           \\
        VAE                & 0          & 1          & 0          & 1           \\
        Full (MoCoO)       & 0          & 0          & 1          & 1           \\
        \bottomrule
    \end{tabular}
\end{table}

At full training scale (3{,}000 cells, 200 epochs), \textbf{VAE+ODE is the dominant configuration}, winning 13 of 27 total metric-beta combinations. Its dominance is strongest at $\beta=1.0$ (7/9 wins) and $\beta=0.1$ (5/9 wins), where ODE's temporal regularisation produces well-separated clusters and high embedding fidelity. At $\beta=0.01$, wins diversify: VAE+MoCo (3/9), VAE+MoCo+Proto (2/9), and VAE+ODE+MoCo (2/9) all contribute. Notably, the Full model only wins 1 metric (NMI at $\beta=0.01$), indicating that at full training scale, the ODE component provides most of the benefit and additional components do not consistently improve over VAE+ODE alone.

\subsection{Biological Validation --- Pseudotime--Marker Correlations}
\label{sec:results:bio}

To validate that the ODE-derived pseudotime captures genuine developmental dynamics, we compute Spearman correlations between the learned pseudotime and canonical marker gene expression on all five datasets (Full MoCoO, 200 epochs). Results for each dataset are presented in Tables~\ref{tab:irall_markers}--\ref{tab:spinoids_markers}, with a cross-dataset summary in Table~\ref{tab:biovalidation}.

%% TABLE VIII
\begin{table}[!t]
    \centering
    \caption{IRALL pseudotime--marker correlations (hematopoiesis).}
    \label{tab:irall_markers}
    \begin{tabular}{llcc}
        \toprule
        Gene & Function & Spearman $\rho$ & $p$-value \\
        \midrule
        \textit{Hbb-bs} & Erythroid hemoglobin     & $+0.275$ & $< 10^{-53}$ \\
        \textit{Hba-a1} & Erythroid hemoglobin     & $+0.264$ & $< 10^{-49}$ \\
        \textit{Elane}  & Granulocyte elastase     & $+0.255$ & $< 10^{-45}$ \\
        \textit{Cd34}   & HSC / progenitor         & $-0.233$ & $< 10^{-38}$ \\
        \textit{Ctsg}   & Granulocyte cathepsin G  & $+0.226$ & $< 10^{-36}$ \\
        \textit{Cd8a}   & T-cell marker            & $-0.168$ & $< 10^{-20}$ \\
        \bottomrule
    \end{tabular}
\end{table}

%% TABLE IX
\begin{table}[!t]
    \centering
    \caption{Dentate pseudotime--marker correlations (neurogenesis).}
    \label{tab:dentate_markers}
    \begin{tabular}{llcc}
        \toprule
        Gene & Function & Spearman $\rho$ & $p$-value \\
        \midrule
        \textit{Slc1a3} & Astrocyte/RGC (GLAST)  & $+0.542$ & $< 10^{-228}$ \\
        \textit{Fabp7}  & Astrocyte/RGC (BLBP)   & $+0.476$ & $< 10^{-169}$ \\
        \textit{Sox2}   & Neural stem cell        & $+0.439$ & $< 10^{-141}$ \\
        \textit{Gfap}   & Astrocyte (GFAP)        & $+0.376$ & $< 10^{-101}$ \\
        \textit{Olig1}  & Oligodendrocyte         & $+0.282$ & $< 10^{-56}$  \\
        \textit{Dcx}    & Migrating neuroblast    & $-0.229$ & $< 10^{-37}$  \\
        \bottomrule
    \end{tabular}
\end{table}

%% TABLE X
\begin{table}[!t]
    \centering
    \caption{Endo pseudotime--marker correlations (pancreatic endocrine).}
    \label{tab:endo_markers}
    \begin{tabular}{llcc}
        \toprule
        Gene & Function & Spearman $\rho$ & $p$-value \\
        \midrule
        \textit{Ins2}    & $\beta$-cell insulin        & $+0.463$ & $< 10^{-134}$ \\
        \textit{Ins1}    & $\beta$-cell insulin        & $+0.372$ & $< 10^{-83}$  \\
        \textit{Neurog3} & Endocrine progenitor        & $-0.319$ & $< 10^{-61}$  \\
        \textit{Pdx1}    & $\beta$-cell / progenitor TF & $+0.280$ & $< 10^{-47}$  \\
        \textit{Chgb}    & Pan-endocrine               & $+0.191$ & $< 10^{-22}$  \\
        \textit{Gcg}     & $\alpha$-cell glucagon      & $+0.181$ & $< 10^{-20}$  \\
        \bottomrule
    \end{tabular}
\end{table}

%% TABLE XI
\begin{table}[!t]
    \centering
    \caption{Paul pseudotime--marker correlations (myeloid/erythroid).}
    \label{tab:paul_markers}
    \begin{tabular}{llcc}
        \toprule
        Gene & Function & Spearman $\rho$ & $p$-value \\
        \midrule
        \textit{Epor}   & Erythropoietin receptor  & $+0.346$ & $< 10^{-77}$ \\
        \textit{Hba-a2} & Erythroid hemoglobin     & $+0.328$ & $< 10^{-69}$ \\
        \textit{Ly6c2}  & Myeloid surface marker   & $+0.272$ & $< 10^{-47}$ \\
        \textit{Elane}  & Granulocyte elastase     & $+0.253$ & $< 10^{-41}$ \\
        \textit{Gata1}  & Erythroid TF             & $+0.231$ & $< 10^{-34}$ \\
        \textit{Gata2}  & Stem/progenitor TF       & $-0.204$ & $< 10^{-27}$ \\
        \bottomrule
    \end{tabular}
\end{table}

Pseudotime aligns with the myeloid/erythroid bifurcation: erythroid markers (\textit{Epor}, \textit{Hba-a2}, \textit{Gata1}) positively correlate while the bipotent progenitor marker \textit{Gata2} anti-correlates, capturing the well-characterised progenitor branching point. The myeloid surface marker \textit{Ly6c2} and granulocyte enzyme \textit{Elane} also show significant positive correlations.

%% TABLE XII
\begin{table}[!t]
    \centering
    \caption{Spinoids pseudotime--marker correlations (spinal cord organoid).}
    \label{tab:spinoids_markers}
    \begin{tabular}{llcc}
        \toprule
        Gene & Function & Spearman $\rho$ & $p$-value \\
        \midrule
        \textit{MKI67}   & Proliferation (Ki-67)         & $+0.492$ & $< 10^{-183}$ \\
        \textit{TOP2A}   & Proliferation (topoisomerase)  & $+0.438$ & $< 10^{-141}$ \\
        \textit{NES}     & Neural progenitor (nestin)     & $+0.198$ & $< 10^{-28}$  \\
        \textit{TUBB3}   & Neuronal $\beta$-tubulin       & $-0.154$ & $< 10^{-17}$  \\
        \textit{SOX2}    & Neural progenitor              & $+0.136$ & $< 10^{-14}$  \\
        \textit{NEUROG1} & Neurogenin-1                   & $-0.076$ & $< 10^{-5}$   \\
        \bottomrule
    \end{tabular}
\end{table}

Pseudotime captures the progenitor-to-differentiated axis in spinal cord organoids: proliferation markers (\textit{MKI67}, \textit{TOP2A}) and progenitor markers (\textit{SOX2}, \textit{NES}) positively correlate, while neuronal marker (\textit{TUBB3}) and neurogenin (\textit{NEUROG1}) anti-correlate, consistent with organoid maturation dynamics.

%% TABLE XIII
\begin{table}[!t]
    \centering
    \caption{Biovalidation summary across all five datasets (Full MoCoO, 200 epochs).}
    \label{tab:biovalidation}
    \resizebox{\columnwidth}{!}{%
    \begin{tabular}{llccl}
        \toprule
        Dataset  & Top Marker      & $\rho$   & $p$-value        & Biological Axis \\
        \midrule
        IRALL    & \textit{Hbb-bs} & $+0.275$ & $< 10^{-53}$     & HSC $\to$ erythroid/granulocyte \\
        Dentate  & \textit{Slc1a3} & $+0.542$ & $< 10^{-228}$    & RGC/stem $\to$ neuroblast \\
        Endo     & \textit{Ins2}   & $+0.463$ & $< 10^{-134}$    & Progenitor $\to$ $\beta$-cell \\
        Paul     & \textit{Epor}   & $+0.346$ & $< 10^{-77}$     & Progenitor $\to$ erythroid \\
        Spinoids & \textit{MKI67}  & $+0.492$ & $< 10^{-183}$    & Proliferating $\to$ differentiated \\
        \bottomrule
    \end{tabular}%
    }
\end{table}

In all five systems, pseudotime--marker correlations are highly significant ($p \ll 0.001$) and biologically interpretable. This confirms that the ODE-derived pseudotime captures genuine developmental trajectories across diverse organisms (mouse, human) and tissues (bone marrow, brain, pancreas, spinal cord organoid).

\subsection{Training Dynamics}
\label{sec:results:training}

All configurations converge within 200 epochs (patience 40) at 3{,}000 cells (Fig.~\ref{fig:training_dynamics}). The VAE trains in ${\sim}27$\,s, MoCo-augmented models in ${\sim}35$\,s, and ODE-containing models in ${\sim}400$\,s (the ODE solver dominates wall-clock time). The computational overhead remains the primary cost of ODE integration (approximately 10--15$\times$ over the VAE baseline). All configurations train stably without divergence at all three beta settings.

\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth,height=0.42\textheight,keepaspectratio]{fig4_training_dynamics}
    \caption{Training dynamics and computational efficiency across all six configurations ($\beta = 0.1$, IRALL, 3{,}000 cells). \textbf{(A)}~Training and validation ELBO loss curves showing convergence within 200 epochs (patience 40); all configurations converge stably with ODE models reaching comparable loss despite higher initial values. \textbf{(B)}~Efficiency scatter: validation ARI vs.\ training time, with bubble size proportional to peak GPU memory. VAE achieves the highest ARI at the lowest cost; ODE-containing models require ${\sim}10\!\times$ more time for comparable clustering quality. The Pareto frontier highlights the accuracy--efficiency trade-off.}
    \label{fig:training_dynamics}
\end{figure*}

\subsection{Batch Integration}
\label{sec:results:batch}

On IRALL (8 batches, d0--d30) at 3{,}000 cells and 200 epochs, ODE-containing configurations achieve better batch mixing (iLISI up to 0.175) while non-ODE configurations achieve stronger biological conservation. Full batch integration analysis is provided in Supplementary Fig.~\ref{fig:supp_batch}; the beta sweep focuses on component interaction analysis.

%% ============================================================
\section{Discussion}
\label{sec:discussion}
%% ============================================================

\subsection{MoCo Standalone Power: Beta-Dependent Effectiveness}
\label{sec:discussion:moco}

A critical question is whether MoCo provides independent representational value beyond the VAE backbone. The beta sweep reveals that \textbf{MoCo's standalone effect is beta-dependent}:

\begin{itemize}
    \item At $\beta = 0.01$ (weak KL), MoCo provides its \textit{strongest} improvement: $\Delta$ARI $= +0.039$, $\Delta$DREX $= +0.006$. With near-zero KL, the contrastive gradient has room to organise the latent space, and instance discrimination compensates for the absent prior structure.
    \item At $\beta = 0.1$ (moderate KL), MoCo has a marginal effect on core clustering: $\Delta$ARI $= -0.019$, $\Delta$ASW $= -0.001$, but contributes positive DREX ($+0.006$), indicating improved embedding fidelity that is not captured by cluster-level metrics.
    \item At $\beta = 1.0$ (strong KL), MoCo hurts: $\Delta$ARI $= -0.021$, $\Delta$ASW $= +0.008$. The KL term dominates, pushing all representations toward the prior, and the contrastive gradient conflicts with this regularisation.
\end{itemize}

This reveals an important design principle: contrastive learning in VAE hybrids requires a sufficiently \textit{weak} KL constraint ($\beta \leq 0.1$) for its instance-level discrimination to manifest. At low $\beta$, MoCo fills the role that KL regularisation cannot---providing structure through pairwise similarity rather than prior matching. The full beta sensitivity profiles across all metrics are visualised in Fig.~\ref{fig:beta_sensitivity}.

\subsection{Prototype Effect: Cluster Compactness and ARI}
\label{sec:discussion:proto}

The prototype contrastive loss (VAE+MoCo $\to$ VAE+MoCo+Proto) shows a \textbf{beta-dependent effect on ARI}: $\Delta$ARI $= +0.048$ at $\beta = 1.0$, but $-0.039$ at $\beta = 0.1$ and $-0.060$ at $\beta = 0.01$. Its effect on Davies--Bouldin is mixed:

\begin{itemize}
    \item $\beta = 1.0$: $\Delta$DB $= -0.029$ (marginal improvement)
    \item $\beta = 0.1$: $\Delta$DB $= +0.113$ (worsens compactness)
    \item $\beta = 0.01$: $\Delta$DB $= +0.118$ (worsens compactness)
\end{itemize}

Prototypes provide a small DRE boost at $\beta \leq 0.1$ ($\Delta$DRE $= +0.015$ at $\beta = 0.1$, $+0.006$ at $\beta = 0.01$), and their ARI contribution is strongest at $\beta = 1.0$ ($+0.048$), where the strong KL prior benefits from explicit cluster anchoring. At low $\beta$, the prototype loss competes with the greater latent freedom, explaining the negative ARI and DB deltas.

\subsection{ODE \texorpdfstring{$\times$}{x} MoCo Synergy: Super-Additive Interaction}
\label{sec:discussion:synergy}

The interaction term analysis (Table~\ref{tab:synergy}) reveals \textbf{regime-dependent synergy} between ODE and MoCo:

\begin{itemize}
    \item \textbf{ARI synergy}: weakly positive at $\beta \geq 0.1$ ($+0.016$, $+0.017$) but negative at $\beta = 0.01$ ($-0.027$). The components cooperate at moderate KL but compete at low KL, where each claims different structural axes.
    \item \textbf{DB synergy}: the strongest signal is at $\beta = 0.01$ ($-0.129$, a large improvement), indicating the combination produces more compact clusters than the sum of individual effects when the latent space is unconstrained.
    \item \textbf{DRE/DREX synergy}: $+0.031$ / $+0.030$ at $\beta = 0.01$. Embedding fidelity improves super-additively at low $\beta$.
\end{itemize}

The mechanism for this synergy is that \textbf{ODE provides trajectory-aligned geometry while MoCo provides instance-level discrimination}. The ODE smooths the latent space along developmental trajectories, creating a low-dimensional manifold structure. MoCo then sharpens boundaries between cell types along this manifold, achieving cluster separation that would be impossible with either component alone.

The DB synergy pattern is notable: harmful at $\beta = 1.0$ ($+0.210$), neutral at $\beta = 0.1$ ($-0.003$), and strongly positive at $\beta = 0.01$ ($-0.129$). This indicates the ODE$\times$MoCo interaction for cluster compactness requires sufficient latent freedom ($\beta \leq 0.01$) to manifest.

\subsection{Beta Sensitivity and Practical Implications}
\label{sec:discussion:beta}

The Full model's performance improves monotonically on most geometric metrics as $\beta$ decreases (Table~\ref{tab:beta_full}): ARI, ASW, CH, DRE, and DREX all improve from $\beta = 1.0$ to $\beta = 0.01$. Only spectral metrics (LSE, LSEX) favor strong KL. This suggests that the standard $\beta = 1.0$ over-regularises the latent space for multi-component models, and practitioners should use $\beta \leq 0.1$ when deploying the Full MoCoO.

\subsection{Component Dominance and Full Model Strengths}
\label{sec:discussion:vae}

At 3{,}000 cells and 200 epochs, VAE+ODE achieves the most metric wins overall (13/27 metric--$\beta$ combinations in Table~\ref{tab:wins}), reflecting the strong regularising effect of Neural ODE on the latent space. The VAE alone wins only 1/27 at full training scale, indicating that additional components are necessary.

While VAE+ODE dominates aggregate metrics, \textbf{the Full model achieves the best distance preservation}---a critical property for downstream visualisation and trajectory analysis. Specifically, the Full model produces the highest DRE distance correlations (UMAP: $0.660$ vs.\ $0.596$ for VAE+ODE; tSNE: $0.725$ vs.\ $0.635$), the highest DREX distance Spearman ($0.716$ vs.\ $0.639$), and the highest Calinski--Harabasz index ($433$ vs.\ $373$). These subcategory advantages, hidden in overall aggregates, indicate that the combination of all components produces latent spaces with the strongest \textit{geometric fidelity}---pairwise distances in the latent space are most faithfully reflected in 2D projections. A detailed subcategory breakdown is provided in Fig.~\ref{fig:subcategory_heatmap}.

\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{fig5_subcategory_heatmap}
    \caption{Subcategory metric breakdown across all six configurations ($\beta = 0.1$, IRALL, 3{,}000 cells). Each panel shows one metric family, with values column-normalised (darker = higher); bold values indicate the best configuration per metric. Win counts (number of best-in-column) are annotated per config. \textbf{(A)}~Clustering: core label-matching metrics. \textbf{(B)}~DRE: dimensionality reduction quality (latent$\to$2D). \textbf{(C)}~DREX: extended DR metrics including distance correlations and rank preservation. \textbf{(D)}~LSE: latent space spectral properties. \textbf{(E)}~LSEX: extended latent metrics including cluster compactness, neighbor purity, sampling stability, and inter-cluster gap. The Full model achieves best scores on distance correlations (B, C) and competitive latent structure (E), while VAE+ODE dominates core clustering (A).}
    \label{fig:subcategory_heatmap}
\end{figure*}

\subsection{Component Contributions --- Summary}
\label{sec:discussion:summary}

The primary effect, best operating regime, and key metrics improved by each component are summarised in Table~\ref{tab:component_summary}. A comprehensive visual overview integrating all evaluation dimensions is provided in Fig.~\ref{fig:composed_benchmark}.

%% TABLE XIV
\begin{table}[!t]
    \centering
    \caption{Component contributions summary.}
    \label{tab:component_summary}
    \resizebox{\columnwidth}{!}{%
    \begin{tabular}{llcl}
        \toprule
        Component & Primary Effect & Best $\beta$ & Key Metrics Improved \\
        \midrule
        \textbf{ODE}          & Trajectory geometry       & All              & DB, CH, ARI \\
        \textbf{MoCo}         & Instance discrimination   & $\mathbf{\beta{=}0.01}$ & ARI, DREX \\
        \textbf{Proto}        & Cluster anchoring         & $\beta{=}1.0$    & ARI, DRE \\
        \textbf{ODE$\times$MoCo} & Regime-dependent synergy & $\beta{=}0.01$   & DB, DRE, DREX \\
        \textbf{Full}         & Distance preservation     & $\beta{\leq}0.1$ & DRE dist.\ corr., DREX Spearman, CAL \\
        \bottomrule
    \end{tabular}%
    }
\end{table}

\subsection{Limitations}
\label{sec:discussion:limitations}

\begin{enumerate}
    \item \textbf{Single dataset for beta sweep:} Only IRALL was tested. Beta sensitivity may differ across datasets. However, validation-to-test generalization remains strong across configurations (Fig.~\ref{fig:generalization}).
    \item \textbf{Single-seed evaluation:} Results are from single training runs; multi-seed evaluation with standard deviations is needed.
    \item \textbf{No external baselines:} Comparison with scVI, scVelo, Harmony is needed to contextualise absolute performance.
    \item \textbf{Clustering metric limitations:} ARI depends on KMeans, which is suboptimal for trajectory-shaped manifolds. Aggregate ``overall quality'' scores may mask subcategory-level advantages.
\end{enumerate}

\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth,height=0.70\textheight,keepaspectratio]{fig5_composed_benchmark}
    \caption{Comprehensive benchmark overview of \MoCoO. \textbf{(A)}~UMAP embeddings of all six configurations, coloured by cell type, showing qualitative clustering improvement from VAE to the Full model. \textbf{(B)}~Radar summary of normalised metrics (ARI, NMI, ASW, DRE, DREX, LSEX) revealing that no single configuration dominates all axes. \textbf{(C)}~Training loss convergence curves. \textbf{(D)}~Latent space diagnostics ($\mu$-norm, $\sigma$-mean, zero-dimension count, pairwise distance). \textbf{(E)}~Learned ODE vector field projected onto UMAP coordinates for the Full model, visualising the continuous dynamics captured by the trajectory module. \textbf{(F)}~Column-normalised heatmap across all evaluation metrics, providing a quantitative bird's-eye comparison.}
    \label{fig:composed_benchmark}
\end{figure*}

%% ============================================================
\section{Conclusion}
\label{sec:conclusion}
%% ============================================================

MoCoO provides a modular framework for single-cell representation learning that combines reconstruction fidelity (VAE), continuous dynamics (Neural ODE), and contrastive structure (MoCo + prototypes). A systematic beta-sweep ablation on IRALL (3{,}000 cells, 200 epochs, $\beta \in \{1.0, 0.1, 0.01\}$) answers three key questions about component interactions:

\textbf{1. ODE is the dominant component.} Neural ODE consistently reduces DB by $0.28$--$0.62$ across all $\beta$ settings and wins 13/27 metric--$\beta$ combinations (Table~\ref{tab:wins}). Its trajectory regularisation produces the most reliably improved latent spaces.

\textbf{2. MoCo has beta-dependent standalone power.} At $\beta = 0.01$, MoCo improves ARI by $+0.039$ and DREX by $+0.006$ over the VAE baseline---contrastive learning is most effective when the KL constraint is weak, allowing instance discrimination to organise the latent space.

\textbf{3. ODE and MoCo exhibit regime-dependent synergy.} The interaction term for DB reaches $-0.129$ at $\beta = 0.01$ (super-additive compactness), with DRE ($+0.031$) and DREX ($+0.030$) synergies confirming improved embedding fidelity. ARI synergy is weakly positive at $\beta \geq 0.1$ ($+0.016$--$0.017$) but turns negative at $\beta = 0.01$.

The Full MoCoO achieves the best \textbf{distance preservation} across all configurations: highest DRE at $\beta = 0.01$ ($0.518$), strongest pairwise distance correlations (DREX Spearman up to $0.72$), and highest Calinski--Harabasz index. While VAE+ODE dominates aggregate clustering metrics, subcategory analysis reveals the Full model uniquely excels at geometric fidelity---the property most critical for faithful 2D visualisation and trajectory inference. Across all settings, pseudotime--marker gene correlations remain biologically significant: \textit{Hbb-bs} ($\rho = 0.28$) in hematopoiesis, \textit{Slc1a3} ($\rho = 0.54$) in neurogenesis, \textit{Ins2} ($\rho = 0.46$) in endocrine pancreas, \textit{Epor} ($\rho = 0.35$) in myeloid/erythroid progenitors, and \textit{MKI67} ($\rho = 0.49$) in spinal cord organoids. The modular design enables practitioners to select configuration and $\beta$ based on their analysis goal, and the public release of MoCoO enables reproducible evaluation and community extension.

%% ============================================================
%% Required Sections (IEEE)
%% ============================================================

\section*{Data Availability}
The IRALL, dentate gyrus, endocrine pancreas, and Paul hematopoiesis datasets are publicly available through the scvelo package~\cite{bergen2020generalizing} and the original publications. The spinoids dataset is available from its original publication. All preprocessed datasets and trained model checkpoints will be made available at \url{https://github.com/PeterPonyu/MoCoO} upon publication.

\section*{Code Availability}
MoCoO is publicly available as an open-source Python package at \url{https://github.com/PeterPonyu/MoCoO} and installable via \texttt{pip install mocoo}. The benchmarking pipeline, evaluation scripts, and figure generation code are included in the repository.

\section*{Declaration of Competing Interest}
The author declares no competing interests.

\section*{Acknowledgment}
The authors acknowledge the use of the University of Birmingham's BlueBEAR HPC service for computational resources.

%% ============================================================
%% Supplementary Figures
%% ============================================================

\appendix

\section{Beta Sensitivity Analysis}
\label{sec:supp:beta}

Fig.~\ref{fig:beta_sensitivity} shows how all evaluation metrics vary across $\beta \in \{0.01, 0.1, 1.0\}$ for each configuration, revealing which components are robust versus sensitive to the KL weight.

\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth,height=0.70\textheight,keepaspectratio]{fig6_beta_sensitivity}
    \caption{Beta sensitivity analysis across all six configurations and nine metric families. Each subplot shows one aggregate metric as a function of $\beta$ (log scale), with one line per configuration. ODE-containing models show the most stable profiles across $\beta$ values, while MoCo and Proto effects are strongly beta-dependent. The COR (correlation) metric is included for completeness but shows high variance across configurations.}
    \label{fig:beta_sensitivity}
\end{figure*}

\section{Generalization: Validation vs.\ Test}
\label{sec:supp:generalization}

Fig.~\ref{fig:generalization} compares validation and held-out test metrics to assess generalization across all configurations at $\beta = 0.1$.

\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth,height=0.70\textheight,keepaspectratio]{fig7_generalization}
    \caption{Generalization assessment: validation (train-split) versus held-out test (test\_ext) metrics for all six configurations ($\beta = 0.1$, IRALL). \textbf{Row 1:} Core clustering metrics (ARI, NMI, ASW). \textbf{Row 2:} Aggregate quality scores (DRE, DREX, LSE, LSEX). Smaller val--test gaps indicate better generalization. The Full model maintains the tightest correspondence on distance preservation metrics (DRE, DREX), while all models generalize well on clustering metrics.}
    \label{fig:generalization}
\end{figure*}

\section{ODE-Driven Pseudotime \& Trajectory Analysis}
\label{sec:supp:ode}

Fig.~\ref{fig:supp_ode} provides detailed analysis of the Neural ODE component's contribution to trajectory inference, including PCA pseudotime comparisons, pseudotime distributions per cell type, gene expression dynamics along pseudotime, and trajectory smoothness metrics.

\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth,height=0.70\textheight,keepaspectratio]{supp_ode_trajectory}
    \caption{ODE-driven pseudotime and trajectory analysis. \textbf{(A)}~PCA of latent spaces coloured by cell type and pseudotime for VAE, VAE+ODE, and Full configurations, showing that ODE adds temporal structure. \textbf{(B)}~Pseudotime distributions per cell type (VAE vs.\ VAE+ODE); the ODE model stratifies developmental stages more continuously. \textbf{(C)}~Top marker gene expression along pseudotime. \textbf{(D)}~Trajectory smoothness: pairwise distance histograms and kNN entropy per configuration.}
    \label{fig:supp_ode}
\end{figure*}

\section{Batch Integration \& Cross-Dataset Generalization}
\label{sec:supp:batch_detail}

Fig.~\ref{fig:supp_batch} provides the full batch integration analysis referenced in Section~\ref{sec:results:batch}, including per-metric bar charts, the bio-conservation vs.\ batch-correction trade-off, cross-dataset heatmaps, and radar profiles.

\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth,height=0.70\textheight,keepaspectratio]{supp_batch_integration}
    \caption{Batch integration and cross-dataset performance. \textbf{(A)}~Batch integration metrics (iLISI, bASW, cLISI, graph connectivity, isolated label ASW) across all configurations on IRALL with 8 time-point batches. \textbf{(B)}~Bio-conservation vs.\ batch-correction trade-off scatter and overall scIB score. \textbf{(C)}~Cross-dataset heatmap of ARI/NMI/ASW. \textbf{(D)}~Cross-dataset radar profile.}
    \label{fig:supp_batch}
\end{figure*}

\section{Biological Validation}
\label{sec:supp:bio}

Fig.~\ref{fig:supp_bio} validates the biological interpretability of MoCoO latent spaces through perturbation analysis, UMAP projections of latent dimensions and gene expression, and per-config gene--component correlation heatmaps.

\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth,height=0.70\textheight,keepaspectratio]{supp_biological_validation}
    \caption{Biological validation of MoCoO latents. \textbf{(A)}~Global perturbation robustness and per-dimension permutation importance. \textbf{(B)}~UMAP projections coloured by top latent dimensions. \textbf{(C)}~Gene expression UMAPs for top RF-importance genes. \textbf{(D)}~Per-config Pearson correlation heatmaps between top genes and latent components.}
    \label{fig:supp_bio}
\end{figure*}

%% ============================================================
%% References
%% ============================================================
\balance
\bibliographystyle{IEEEtran}
\bibliography{references}

\end{document}
